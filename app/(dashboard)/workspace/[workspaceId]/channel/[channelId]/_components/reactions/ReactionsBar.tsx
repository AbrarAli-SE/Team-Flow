...
export function ReactionsBar({ messageId, reactions, context }: ReactionsBarProps) {
                ...
                if (isThread) {
                    ...
                    queryClient.setQueryData(listOptions.queryKey,
                        (old) => {
                            ...
                            return {
                                ...old,
                                messages: old.messages.map((m) => m.id === vars.messageId ? {
                                    ...m,
    ...
    const handeToggle = (emoji: string) => {
    ...
    return (
        <div className="mt-1 flex items-center gap-1">
            {reactions.map((r) => (
                <Button
                    key={r.emoji} type="button" size="sm" variant="secondary"
                    className={cn(
                        "h-6 px-2 text-xs",
                        r.reactedByMe && "bg-primary/10 border-primary border"
                    )} onClick={() => handeToggle(r.emoji)}>
                    <span>{r.emoji} </span>
                    <span>{r.count}</span>

                </Button>
            ))}
            <EmojiReaction onSelect={handeToggle} />
        </div>
    )
} 