...
import { createMessageSchema, GroupedReactionsSchema, GroupReactionsSchemaType, toggleReactionSchema, updateMessageSchema } from "../schemas/message";
  ...
  for (const reaction of reactions) {
    ...
    if (existing) {
      ...
      reactionMap.set(reaction.emoji,
        { count: 1, reactedByMe: reaction.userId === userId });
    }
  }
...
export const createMessage = base
  ...
  .handler(async ({ input, context, errors }) => {
    ...
    // if this a thread reply, validate the parent message
    if (input.threadId) {
      ...
      if (!parentMessage || parentMessage.channelId !== input.channelId || parentMessage.threadId !== null) {
        throw errors.BAD_REQUEST({ message: "Invalid thread ID" });
      }
    }
...
export const updateMessage = base.
  ...
  .handler(async ({ input, context, errors }) => {
    ...
    if (!message) {
      throw errors.NOT_FOUND({ message: "Message not found" });
    }

    if (message.authorId !== context.user.id) {
      throw errors.FORBIDDEN({ message: "You do not have permission to edit this message" });
    }
...
export const listThreadReplies = base
  ...
  .handler(async ({ input, context, errors }) => {
    ...
    if (!parentRow) {
      throw errors.NOT_FOUND({ message: "Invalid thread ID" });
    }

    // fetch all replies to this thread
