...
export const generateThreadSummary = base
    ...
    .route({
        method: "GET",
        path: "/ai/thread/summary/generate",
    ...
    .handler(async ({ input, context, errors }) => {
        ...
        if (replies.length > 0) {
            ...
            for (const r of replies) {
                ...
                lines.push(
                    `\- ${r.authorname} - ${r.createdAt.toISOString()}: ${t}`);
            }
        }
        const compiled = lines.join("\n");

        const system = [" You are an expert assistant summarizing Slack - like discussion threads for a product team.",
...
export const generateCompose = base
    ...
    .handler(async ({ input }) => {
        ...
        const system = [
            ...
            "Task: Rewrite the provided content to be clear and better structured while preserving meaning, facts, terminology, and names.",
            ...
            "Return only the rewritten content. No headings or closing remarks.",
        ].join("\n");
