...
export function ComposeAssistant({ content, onAccept }: ComposeAssistantProps) {
    ...
    const composeText = lastAssistant?.parts.filter((part) => part.type === "text").map((part) => part.text).join("\n\n") ?? "";
            ...
            <PopoverContent className="w-[25rem] p-0" align="end">
                ...
                <div className="px-4 py-3 max-h-80 overflow-y-auto">
                    {error ? (
                        ...
                        <Response partialMarkdown={status !== "ready"}>{composeText}</Response>
                ...
                <div className="flex items-center justify-end gap-3 border-top px-3 py-2 bg-muted/30">
                    <Button
                        type="submit"
                        size="sm"
                        variant="outline"
